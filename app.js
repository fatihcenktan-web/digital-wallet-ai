...
const appState = {
    user: { name: 'Loading...', balance: 56078.00 },
    banks: [],
    notifications: [],
    transactions: [],
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderUI = () => {
        const balanceToDisplay = (appState.user.balance !== undefined && appState.user.balance !== null && !isNaN(appState.user.balance)) ? appState.user.balance : 56078.00;
        document.getElementById('sidebarUserName').innerText = appState.user.name;
        document.getElementById('totalBalance').setAttribute('data-val', balanceToDisplay);
    ...
    window.showUIToast = (message, icon = 'info') => {
        const toast = document.createElement('div');
        toast.className = 'session-toast';
        // Override session-toast red colors to premium indigo/primary
        toast.style.position = 'relative';
        toast.style.bottom = 'auto';
        toast.style.left = 'auto';
        toast.style.transform = 'none';
        toast.style.background = 'rgba(99, 102, 241, 0.2)';
    ...
    // Make all dummy buttons interactive
    setTimeout(() => {
        ...
        allButtons.forEach(btn => {
            ...
            if (btn.classList.contains('interactive-icon') || btn.hasAttribute('data-action-bound')) return;
            // Skip filter buttons (they are handled in views)
